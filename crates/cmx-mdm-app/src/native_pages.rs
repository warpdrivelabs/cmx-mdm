//! 模型中心微服务自持的 native-pages / html-pages 只读投递（F2：一芯双壳的前端页面壳）。
//!
//! 门户的 native/html page 是 **API-backed**（非静态文件）：源码存 `web/ui-native/sources/<relPath>`
//! （html 存 `web/ui-html/sources/<relPath>`），经 `GET /api/native-pages/{id}` 读出
//! `ApiResp<NativePageFull>` 返回，shell 用 API 取页面内容。本模块让 model-server 用**字节对齐门户
//! 的信封**自投递模型中心的 native/html 页——门户 F3 反代 `/api/native-pages/{model-id}` 到本服务时，
//! 响应与门户内嵌路径逐字节一致，shell 零感知。
//!
//! ⚠ 与 cmx-report 的 native_pages.rs 唯一差异：模型中心页面 index 里的 `relPath` 相对 `sources/`
//! 子目录（`<ui_dir>/sources/<relPath>`），而非直接落 ui_dir。这与 cmx-container/data/native-pages
//! 的原始磁盘布局一致（index.json 与 sources/ 平级）。
//!
//! 契约（对齐 cmx-common-api/portal/pages.rs + cmx-form/pages/{native,html}.rs）：
...
/// 把可能为 JSON `null` 的字符串字段反序列化为空串（serde `default` 只覆盖**缺失**字段，
/// 不覆盖显式 `null`——门户页面清单里 `doc:null` 等写法会让整个分片解析失败并静默降级为空）。
/// 与门户 cmx-form/pages 的宽松读取对齐：null/缺失 → 空串。
...
/// 安全拼接源文件路径（禁止 `..` 越界，relPath 只能落在 `<ui_dir>/sources/` 内）。
...
// ============================================================================
// handlers（无 state；对任意泛型 S 成立，与 model_routes 同构）
// ============================================================================
...
/// native-pages 只读路由（挂在 model-server 的 /api 下，与门户前缀一致）。
...
/// 安全拼接 html 源文件绝对路径（禁止 `..` 越界；落 `<html_dir>/sources/`）。
...
/// 前端页面只读路由（native + html 并列，挂 model-server /api 下，前缀与门户一致）。
/// 门户 F3 反代模型中心拥有的 native/html 页取页请求到本服务；独立运行时也自投递自己的界面。
